...
// .wrangler/tmp/bundle-6ifW24/checked-fetch.js
...
// src/services/hospitable.ts
var HospitableService = class {
  static {
    __name(this, "HospitableService");
  }
  token;
  cache;
  baseUrl;
  ...
  async getProperty(id) {
    try {
      const response = await this.fetch(`/properties/${id}`, { ttl: 300 });
  ...
  async getReservations(params) {
    ...
    const response = await this.fetch(endpoint, { ttl: 120 });
  ...
  async getReservation(id) {
    try {
      const response = await this.fetch(`/reservations/${id}`, { ttl: 120 });
   ...
   */
  async fetchAllPages(endpoint, delayMs = 1e3) {
    const allData = [];
   ...
   */
  extractPropertyNumber(name) {
    const match2 = name.match(/^(\d{1,3})/);
    if (match2 && match2[1]) {
   ...
   */
  filterBkkProperties(properties2) {
    return properties2.filter((property) => {
  ...
  async syncAllProperties() {
    const allProperties = await this.fetchAllPages("/properties", 1e3);
    const filteredProperties = this.filterBkkProperties(allProperties);
    const properties2 = filteredProperties.map((p) => ({
      id: p.id,
      hospitable_id: p.id,
      name: p.name || p.nickname || "Unnamed Property",
      internal_code: p.internal_code || p.id.slice(0, 8),
    ...
    const cacheKey = "properties:bkk:filtered";
    await this.cache.put(cacheKey, JSON.stringify(properties2), {
      expirationTtl: 86400
      // 24 hours
    });
    await this.cache.put("properties:bkk:metadata", JSON.stringify({
      ...
      filteredCount: properties2.length
    }), {
      expirationTtl: 86400
    });
    return {
      total: allProperties.length,
      filtered: properties2.length,
      cached: true
    };
  }
};
...
reservations.get("/", async (c) => {
  ...
  try {
    ...
    const properties2 = await service.getProperties();
      ...
      const batchPromises = batch.map(
        (id) => service.getReservations({
          ...
          (reservations2) => (
            ...
            reservations2.map((r) => ({ ...r, property_id: id }))
          )
        ).catch((err) => {
...
var reservations_default = reservations;

// src/services/turno.ts
var TurnoService = class {
  static {
    __name(this, "TurnoService");
  }
  apiKey;
  cache;
  baseUrl;
  ...
  async getCleaningJobs(params) {
    ...
    try {
      const response = await this.fetch(endpoint, { ttl: 60 });
      return response.data.map((job) => this.mapJob(job));
    } catch (err) {
      console.error("Turno API error:", err);
      return [];
    }
  }
  async getCleaningJob(id) {
    try {
      const response = await this.fetch(`/jobs/${id}`, { ttl: 60 });
...
var cleaning_default = cleaning;

// src/routes/tasks.ts
var tasks = new Hono2();
async function getTaskIds(kv) {
  const index = await kv.get("tasks:index", "json");
  return index || [];
}
__name(getTaskIds, "getTaskIds");
async function updateTaskIndex(kv, ids) {
  await kv.put("tasks:index", JSON.stringify(ids));
}
__name(updateTaskIndex, "updateTaskIndex");
tasks.get("/", async (c) => {
...
dashboard.get("/stats", async (c) => {
  ...
  const kv = c.env.TASKS;
  try {
    const properties2 = await hospitable.getProperties();
    const propertyIds = properties2.map((p) => p.id);
    const batchSize = 10;
    const allReservations = [];
    for (let i = 0; i < propertyIds.length; i += batchSize) {
      const batch = propertyIds.slice(i, i + batchSize);
      const batchPromises = batch.map(
        (id) => hospitable.getReservations({ property_id: id }).catch(() => [])
    ...
    const [cleaningJobs, taskIds] = await Promise.all([
      ...
      // Turno may be blocked by Cloudflare
      kv.get("tasks:index", "json")
    ]);
...
dashboard.get("/upcoming", async (c) => {
  ...
  try {
    ...
    const properties2 = await hospitable.getProperties();
    const propertyMap = new Map(properties2.map((p) => [p.id, p]));
    const propertyIds = properties2.map((p) => p.id);
    const batchSize = 10;
    const allReservations = [];
    for (let i = 0; i < propertyIds.length; i += batchSize) {
      const batch = propertyIds.slice(i, i + batchSize);
      const batchPromises = batch.map(
        (id) => hospitable.getReservations({
          from: today,
          to: nextWeek,
          property_id: id
        }).then(
          (reservations2) => reservations2.map((r) => {
    ...
    const upcoming = allReservations.filter((r) => r.status !== "cancelled" && r.check_in_date >= today).sort((a, b) => new Date(a.check_in_date).getTime() - new Date(b.check_in_date).getTime()).slice(0, limit);
...
dashboard.get("/today-cleaning", async (c) => {
  ...
  try {
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const [properties2, jobs] = await Promise.all([
...
dashboard.get("/occupancy-trends", async (c) => {
  ...
  const dailyOccupied = /* @__PURE__ */ new Map();
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    const dateKey = d.toISOString().split("T")[0];
    dailyOccupied.set(dateKey, /* @__PURE__ */ new Set());
  }
  ...
  const result = Array.from(dailyOccupied.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([dateStr, occupiedProperties]) => {
    const date = new Date(dateStr);
    const month = date.toLocaleString("en-US", { month: "short" });
    const day = date.getDate().toString().padStart(2, "0");
    const rate = totalProperties > 0 ? Math.round(occupiedProperties.size / totalProperties * 100 * 10) / 10 : 0;
    return {
      date: `${month} ${day}`,
      rate
    };
...
dashboard.get("/revenue-trends", async (c) => {
  ...
  const result = Array.from(dailyData.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([date, data]) => ({
    date,
    revenue: Math.round(data.revenue * 100) / 100,
...
dashboard.get("/property-performance", async (c) => {
  ...
  const result = Array.from(performanceMap.entries()).map(([propertyId, data]) => {
    const property = properties2.find((p) => p.id === propertyId);
    return {
      name: property?.name || "Unknown",
      revenue: Math.round(data.revenue * 100) / 100,
      bookings: data.bookings
    };
  }).filter((item) => item.revenue > 0).sort((a, b) => b.revenue - a.revenue).slice(0, limit);
  return c.json({ data: result });
});
dashboard.get("/task-priority-breakdown", async (c) => {
  const kv = c.env.TASKS;
  try {
    const taskIds = await kv.get("tasks:index", "json");
    ...
    const tasks2 = await Promise.all(
      taskIds.map((id) => kv.get(`task:${id}`, "json"))
    );
...
dashboard.get("/recent-activity", async (c) => {
  ...
  allReservations.forEach((reservation) => {
    if (reservation.status !== "cancelled") {
      ...
      const createdTime = new Date(reservation.check_in_date).getTime() - 7 * 24 * 60 * 60 * 1e3;
      activities.push({
        type: "booking",
  ...
  cleaningJobs.forEach((job) => {
    ...
    activities.push({
      type: "cleaning",
      property: propertyName,
      description: job.status === "completed" ? `Cleaning completed by ${job.cleaner_name || "cleaner"}` : `Cleaning ${job.status} for ${job.cleaner_name || "cleaner"}`,
      timestamp,
  ...
  const result = activities.sort((a, b) => b.sortKey - a.sortKey).slice(0, limit).map((activity) => {
...
app.route("/api/reservations", reservations_default);
app.route("/api/cleaning", cleaning_default);
...
// .wrangler/tmp/bundle-6ifW24/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6ifW24/middleware-loader.entry.ts
